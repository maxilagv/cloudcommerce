        ...
        {homePromos.map((promo, index) => {
          ...
          return (
            <Link
              key={promo.id}
              href={promo.href}
              className={`cc-focus-ring group relative overflow-hidden rounded-[24px] border p-5 shadow-cc-xs transition-[transform,border-color,box-shadow] duration-[220ms] ease-cc-out hover:-translate-y-[3px] hover:shadow-cc-md ${toneClass[promo.tone]} ${primary ? "min-h-[210px] lg:col-span-2" : "min-h-[170px]"}`}
            >
