...
export function CategoryShowcase({ categories }: { categories: HomeCategory[] }) {
  ...
  return (
    <section id="home-categories" aria-labelledby="home-categories-title" className="mt-8">
      ...
      />
      <RevealGroup className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        {categories.map(({ id, title, description, href, image, icon: Icon, accent }) => (
          <motion.div key={id} variants={fadeSlideUp}>
            <Link
              href={href}
              className={`cc-focus-ring group relative block min-h-[154px] overflow-hidden rounded-[22px] border border-cc-border bg-gradient-to-br ${accentClass[accent]} p-4 shadow-cc-xs transition-[transform,border-color,box-shadow] duration-[220ms] ease-cc-out hover:-translate-y-[3px] hover:border-cc-primary-border hover:shadow-cc-md`}
            >
