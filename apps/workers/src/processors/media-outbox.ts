...
async function processOne(eventId: string, payload: Record<string, unknown>): Promise<void> {
  ...
  await db.update(outboxEvent).set({ status: "processing", attempts: drizzleSql`${outboxEvent.attempts} + 1` }).where(eq(outboxEvent.id, eventId));
...
async function markFailed(eventId: string, reason: string): Promise<void> {
  await db.update(outboxEvent).set({ status: "failed", lastError: reason }).where(eq(outboxEvent.id, eventId));
}
